...
def create_app(project_dir: Path | None = None, debug: bool = False) -> FastAPI:
    ...
    if not brain_initialized:
        logger.warning(
            f"Brain not initialized in {project_dir or Path.cwd()}. Some tools may not work."
        )

    # Check vector store availability
    vector_available = False
    try:
        from oxmemory.storage.vector_store import VectorStore  # noqa: F401
