...
def get_brain_path(project_dir: Path | None = None) -> Path:
...
def get_default_config(project_name: str | None = None) -> Config:
...
def _interpolate_env_vars(data: dict) -> dict:
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            result[key] = [
                _interpolate_env_vars(item) if isinstance(item, dict) else item for item in value
            ]
...
def load_config(project_dir: Path | None = None) -> Config:
    ...
    if not config_path.exists():
        raise FileNotFoundError(
            f"Config file not found: {config_path}\nRun '0xmemory init' to create a new brain."
        )
...
def save_config(config: Config, project_dir: Path | None = None) -> Path:
...
def brain_exists(project_dir: Path | None = None) -> bool:
