...
- **Batteries-Included**: Restored full local AI capability by default (includes `chromadb` and `sentence-transformers`).
- **Hybrid Architecture**: Vector search runs locally (fast/private), while Knowledge Extraction uses APIs (via `litellm`).
- **Features**: Semantic Search and Auto-Tagging work out-of-the-box.
